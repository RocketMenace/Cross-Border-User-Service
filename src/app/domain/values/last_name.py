...
from app.domain.exceptions import InvalidLastNameError
...
class LastName(BaseValueObject):
    ...
    def _validate(self, *, value: str):
        if not isinstance(value, str):
            raise ValueError(
                f"Last name must be a string, got {type(self.value).__name__}",
            )
    ...
    def _validate_pattern(self, *, value: str):
        if not value.istitle():
            raise InvalidLastNameError(f"Last name must start with capital letter. Got: {value}")
        if not self.PATTERN.match(value):
            raise InvalidLastNameError(
                f"Last name must contain only letters and hyphens. Got: '{value}'",
            )
